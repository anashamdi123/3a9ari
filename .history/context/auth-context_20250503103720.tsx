...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (session?.user) {
        ...
        setSessionStartTime(Date.now()); // Set session start time when user logs in
    ...
    const checkSessionExpiration = async () => {
      if (sessionStartTime) {
        ...
        if (elapsedTime >= TEST_SESSION_DURATION) {
          console.log('Session expired after 10 seconds');
          await logout();
        }
      }
    };
  ...
  async function checkUser() {
    try {
      ...
      if (data.session?.user) {
        ...
        setSessionStartTime(Date.now()); // Set session start time when checking user
  ...
  async function login(email: string, password: string) {
    try {
      ...
      setSessionStartTime(Date.now()); // Set session start time on login
      return { error: null };
    } catch (error: any) {
      return { 
        error: { 
  ...
  async function register(email: string, password: string, fullName: string) {
    try {
      ...
      setSessionStartTime(Date.now()); // Set session start time on register
      return { error: null };
    } catch (error: any) {
      return { 
        error: { 
