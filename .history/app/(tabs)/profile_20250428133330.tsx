...
export default function ProfileScreen() {
  ...
  const [formData, setFormData] = useState({
    full_name: (profile as ExtendedUserProfile)?.full_name || '',
    phone_number: (profile as ExtendedUserProfile)?.phone_number || '',
    location: (profile as ExtendedUserProfile)?.location || '',
  });
  ...
  const handleMyListings = () => {
    router.push('/(tabs)/profile/listings');
  };
          ...
          <View style={styles.avatarContainer}>
            <View style={styles.avatar}>
              <Text style={styles.avatarText}>
                {extendedProfile?.full_name ? extendedProfile.full_name[0].toUpperCase() : '?'}
              </Text>
            </View>
            <View style={styles.profileInfo}>
              <Text style={styles.profileName}>{extendedProfile?.full_name || 'المستخدم'}</Text>
              <Text style={styles.profileEmail}>{extendedProfile?.email || ''}</Text>
            </View>
          </View>
            ...
            <>
              <View style={styles.infoSection}>
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoText}>{extendedProfile?.email}</Text>
                </View>
                
                {extendedProfile?.phone_number && (
                  <View style={styles.infoRow}>
                    <Phone size={20} color={Theme.colors.text.secondary} />
                    <Text style={styles.infoText}>{extendedProfile.phone_number}</Text>
                  </View>
                )}
                ...
                {extendedProfile?.location && (
                  <View style={styles.infoRow}>
                    ...
                    <Text style={styles.infoText}>{extendedProfile.location}</Text>
                  </View>
                )}
                ...
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoText}>
                    تاريخ الانضمام: {new Date(extendedProfile?.created_at || '').toLocaleDateString('ar-SA')}
                  </Text>
                </View>
              </View>
