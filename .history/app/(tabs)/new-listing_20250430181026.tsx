...
export default function NewListingScreen() {
  ...
  // Form state
  const [images, setImages] = useState<{ uri: string; size: number }[]>([]);
    ...
    if (!result.canceled && result.assets.length > 0) {
      const newImages = await Promise.all(
        result.assets.map(async (asset) => {
          ...
          if (size > 5 * 1024 * 1024) {
            setToast({ 
              message: `تم تجاهل الصورة ${asset.fileName} لأنها تتجاوز الحد الأقصى للحجم (5MB)`, 
              type: 'error' 
            });
            return null;
          }
      ...
      const validImages = newImages.filter((img): img is { uri: string; size: number } => img !== null);
      
      if (validImages.length > 0) {
        setImages([...images, ...validImages]);
        const totalSize = validImages.reduce((sum, img) => sum + img.size, 0);
        setToast({ 
          message: `تم إضافة ${validImages.length} صورة بنجاح (${formatFileSize(totalSize)})`, 
          type: 'success' 
        });
      }
    }
