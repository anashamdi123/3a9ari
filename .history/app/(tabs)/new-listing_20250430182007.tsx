...
export default function NewListingScreen() {
  ...
  const handlePickImage = async () => {
    if (images.length >= 12) {
      Alert.alert('تنبيه', 'يمكنك إضافة 12 صورة كحد أقصى');
      return;
    }
    ...
    if (!result.canceled && result.assets.length > 0) {
      ...
      const totalNewSize = newImages.reduce((sum, img) => sum + img.fileSize, 0);
      const newTotalSize = totalSize + totalNewSize;

      if (newTotalSize > 60 * 1024 * 1024) { // 60MB total limit (12 images × 5MB)
        Alert.alert('تنبيه', 'حجم الصور المحددة يتجاوز الحد المسموح به');
        return;
      }

      // Check individual file sizes
      const oversizedImages = newImages.filter(img => img.fileSize > 5 * 1024 * 1024);
      if (oversizedImages.length > 0) {
        Alert.alert('تنبيه', 'بعض الصور المحددة تتجاوز حجم 5 ميجابايت');
        return;
      }

      setImages([...images, ...newImages.map(img => img.uri)]);
  ...
  const handleRemoveImage = (index: number) => {
    ...
    const newTotalSize = newImages.reduce((sum, uri) => {
      // Note: This is an approximation since we don't have the actual file size after removal
      // In a real app, you'd want to store the file sizes along with the URIs
      return sum + (5 * 1024 * 1024); // Using 5MB as an average
    }, 0);
    setTotalSize(newTotalSize);
  };
          ...
          <View style={styles.stepContainer}>
            ...
            <View style={styles.sizeInfoContainer}>
              <Text style={styles.sizeInfoText}>
                عدد الصور: {images.length}/12
              </Text>
              <Text style={styles.sizeInfoText}>
                الحجم الإجمالي: {(totalSize / (1024 * 1024)).toFixed(2)} ميجابايت
              </Text>
            </View>
          </View>
        )}
